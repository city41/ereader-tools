...
let sizetree = null;
let log = [];
function log_write(msg, ...args) {
    log.push(JSON.stringify(msg, args));
    // console.log(msg, args);
}
function get_bitsize(data) {
    let i = data;
    let size = 0;
    while (i > 0) {
        size++;
...
function write_bits(data, count, f) {
    bits_written += count;
    if (!f && !bitstore) {
        return;
    }
...
function get_treenode(tree, bitcount, numbits = 0, value = 0, forcelookup = 0) {
    let i;
    let tmp = null;
    if (forcelookup == 1) {
        if (tree.node == 1) {
            tmp = get_treenode(tree.left, bitcount, numbits + 1, value + (0 << numbits), 1);
            if (tmp === null) {
                tmp = get_treenode(tree.right, bitcount, numbits + 1, value + (1 << numbits), 1);
                if (tmp === null)
                    return null;
                else
                    return tmp;
        ...
        return tree.look_up[bitcount];
    }
}
function sort_tree(tree, count) {
    for (let i = 0; i < count; i++)
...
function write_huffman_tree(tree, f, root = 1) {
    if (tree.node == 1) {
        if (f)
            log_write("(");
        ...
        if (root)
            write_bits(1, 1, f);
        if (f)
            log_write("%d", tree.data);
        if (root)
            if (f)
                log_write("\n");
    }
}
function create_huffman_tree(buf, count, f, method = 0, type = 0) {
    let treenodes = 0;
    let ptr = null;
    let temp1 = null;
    let treeroot = null;
    ...
    while (treenodes > 1) {
        ...
        for (let i = treenodes; i >= 0; i--) {
            temp1 = pointer[i];
            if (temp1.count < ptr.right.count)
                ptr.right = temp1;
        }
        for (let i = treenodes; i >= 0; i--) {
            if (ptr.right == pointer[i])
                continue;
            ptr.left = pointer[i];
            break;
        }
...
function compress(buf, size, compression_level, lzwindow, lzsize, method, f, bitdata = null) {
    const literals = new Array(MAX_LITERALS);
    const move_t = new Array(MAX_TABLE_SIZE);
    const size_t = new Array(MAX_TABLE_SIZE);
    let literals_offset = 0, move_offset = 0, size_offset = 0;
    ...
    if (!skip_lz77) {
        if (compression_level !== 0) {
            ...
            for (buf_offset = 0; buf_offset < size; buf_offset++) {
                if (literals_offset == MAX_LITERALS ||
                ...
                for (buf_back_offset =
                    buf_offset < lzwindow ? buf_offset - 1 : lzwindow - 1; buf_back_offset >= 0; buf_back_offset--) {
                    i = buf_offset;
                    j = buf_back_offset;
                    k = 0;
            ...
            if (literals[literals_offset] == 0x100) {
                ...
                if (method == 1) {
                    ...
                    if (tmp != null) {
                        ...
                        write_bits(move_t[move_offset++], movetree.data, f);
                }
                ...
                if (tmp != null) {
                    ...
                    write_bits(size_t[size_offset++], sizetree.data, f);
            }
            else {
                write_bits(0, 1, f);
                write_bits(literals[literals_offset], 8, f);
