...
function initSav(): number[] {
  const savData: number[] = new Array(SAV_SIZE).fill(0);

  function memset(start: number, value: number, count: number) {
    savData.splice(start, count, ...new Array(count).fill(value));
  }

  function memcpy(start: number, data: number[]) {
...
function createSavFile(
  binary: number[],
  type: CardType,
  region: CardRegion,
  name: string
) {
  const savData: number[] = initSav();
  ...
  if (isVpk) {
    ...
    if (pos + binary.length > 0x1f000) {
      ...
      memcpy(
        0,
        binary.slice(0x1f000 - pos, 0x1f000 + binary.length - (0x1f000 - pos))
      );
      overflow = 1;
    } else {
      memcpy(pos, binary);
  ...
  if (overflow) {
    ...
    crc = crc32(
      savData.slice(0x10004, 0x10004 + 0x1f000 - 0x10004),
      0xaa478422
    );
    // crc = crc32(datadst + 0, size_vpk + 0x30 - (0x1f000 - 0x10004), crc);
    crc = crc32(savData.slice(0, size_vpk + 0x30 - (0x1f000 - 0x100004)), crc);
  } else {
    // crc = crc32(datadst + 0x10004, size_vpk + 0x30, 0xaa478422);
    crc = crc32(savData.slice(0x10004, 0x10004 + size_vpk + 0x30), 0xaa478422);
  }
