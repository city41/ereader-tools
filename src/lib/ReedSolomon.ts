class _ReedSolomon {
  static d_mm = 8; /* RS code over GF(2**4) - change to suit */
  static d_nn = 255; /* nn=2**mm -1   length of codeword */
  static d_tt = 127; /* number of errors that can be corrected */
  static d_kk = _ReedSolomon.d_nn - 2 * _ReedSolomon.d_tt; /* kk = nn-2*tt  */
  ...
  private zerofill(data: number[], len: number) {
    for (let i = 0; i < len; ++i) {
      data[i] = 0;
    }
  }
...
in R. Blahut's "Theory ... of Error-Correcting Codes". */
  ...
  private eras_dec_rs(eras_pos: number[], no_eras: number) {
    ...
    if (no_eras > 0) {
      ...
      for (let i = 1; i < no_eras; i++) {
        ...
        for (let j = 1; j < i + 2; j++) phi[j] = phi[j] ^ tmp_pol[j];
      }
 ...
 @**(b0+i), i = 0, ... ,(2*tt-1) */
    for (let i = 1; i <= this.nn - this.kk; i++) {
      ...
      for (let j = 0; j < this.nn; j++)
        if (this.recd[j] != 0xff)
          s[i] ^=
            this.alpha_to[
              (this.recd[j] + (this.b0 + i - 1) * j) % 0xff
    ...
    if (syn_error) {
      ...
      if (no_eras > 0) {
        ...
        for (let i = deg_phi + 1; i < 2 * this.tt + 1; i++) lambda[i] = 0;
        // let deg_lambda = deg_phi;
        for (let i = 0; i < 2 * this.tt + 1; i++) b[i] = lambda[i];
      } else {
        lambda[0] = 1;
        for (let i = 1; i < 2 * this.tt + 1; i++) lambda[i] = 0;
        for (let i = 0; i < 2 * this.tt + 1; i++) b[i] = lambda[i];
      }
      while (++r <= 2 * this.tt) {
        ...
        for (let i = 0; i < 2 * this.tt + 1; i++) {
          if (lambda[i] != 0 && s[r - i] != 0xff) {
            let tmp =
              this.alpha_to[(this.index_of[lambda[i]] + s[r - i]) % 0xff];
            discr_r ^= tmp;
          }
        }
        if (discr_r == 0) {
          ...
          tmp_pol[0] = 0;
          for (let i = 1; i < 2 * this.tt + 1; i++) tmp_pol[i] = b[i - 1];
          for (let i = 0; i < 2 * this.tt + 1; i++) b[i] = tmp_pol[i];
        } else {
          ...
          for (let i = 1; i < 2 * this.tt + 1; i++) {
            let tmp =
              b[i - 1] == 0
                ? 0
                : this.alpha_to[
                    (this.index_of[discr_r] + this.index_of[b[i - 1]]) % 0xff
                  ];
            T[i] = lambda[i] ^ tmp;
          }
            ...
            for (let i = 0; i < 2 * this.tt + 1; i++)
              b[i] =
                lambda[i] == 0
                  ? 0
                  : this.alpha_to[
                      (this.index_of[lambda[i]] -
                        this.index_of[discr_r] +
                        this.nn) %
                        0xff
                    ];
            for (let i = 0; i < 2 * this.tt + 1; i++) lambda[i] = T[i];
          } else {
            for (let i = 0; i < 2 * this.tt + 1; i++) lambda[i] = T[i];
            /* 3 lines below: B(x) <-- x*B(x) */
            tmp_pol[0] = 0;
      ...
      while (lambda[deg_lambda] == 0xff && deg_lambda > 0) --deg_lambda;
      if (deg_lambda <= 2 * this.tt) {
        /* Find roots of the error+erasure locator polynomial. By Chien Search */
        for (let i = 1; i < 2 * this.tt + 1; i++) reg[i] = lambda[i];
        ...
        if (deg_lambda == count) {
          ...
          while (omega[deg_omega] == 0 && deg_omega > 0) --deg_omega;
  ...
  num2 = inv(X(l))**(b0-1) and den = lambda_pr(inv(X(l))) all in poly-form */
          for (let j = 0; j < count; j++) {
            ...
            for (let i = 0; i < deg_omega + 1; i++) {
              ...
              if (omega[i] != 0)
                tmp =
                  this.alpha_to[
                    (this.index_of[omega[i]] + i * pres_root) % 0xff
                  ];
              else tmp = 0;
              num1 ^= tmp;
            }
            ...
            for (let i = 0; i < deg_lambda + 1; i++) {
              ...
              if (lambda_pr[i] != 0)
                tmp =
                  this.alpha_to[
                    (this.index_of[lambda_pr[i]] + i * pres_root) % 0xff
                  ];
              else tmp = 0;
              den ^= tmp;
            }
          ...
          for (let j = 0; j < count; j++) this.recd[loc[j]] ^= err[loc[j]];

          return count;
        } /* deg(lambda) unequal to number of roots => uncorrectable error detected */ else
          return -2;
      } /* deg(lambda) > 2*tt => uncorrectable error detected */ else return -3;
    } else {
  ...
  private make_pow(data: number[], len: number) {
    for (let i = 0; i < len; i++) data[i] = this.alpha_to[data[i]];
  }

  private make_rev(data: number[], len: number) {
    for (let i = 0; i < len; i++) data[i] = this.index_of[data[i]];
  }

  private gen_poly(errlen = 16) {
...
*/
  ...
  private reverse_byte_order(data: number[], len: number) {
  ...
  private invert_error_bytes(data: number[], len: number) {
    for (let i = 0; i < len; i++) data[i] ^= 0xff;
  }

  correct_errors(
    data: number[],
    errlen: number,
    erasure: number[] | null = null
  ) {
    let result = 0;
    let j = 0;
